...
/**
 * Example: Carbon AI Chat — Custom element (Sidebar)
 *
 * Demonstrates: rendering Carbon AI Chat as a docked side panel by mounting
 * `ChatCustomElement` inside a host `<div>` styled with the library's shipped
 * `cds-aichat-sidebar` layout classes. A host header bar carries a toggle
 * button that opens and closes the panel; the `VIEW_CHANGE` / `VIEW_PRE_CHANGE`
 * lifecycle hooks drive the slide-in and slide-out animation so the chat only
 * unmounts once the CSS transition ends.
 *
 * APIs exercised:
