 ...
 * atom whose node view renders a Carbon `<cds-tile>`. Because its type name
 ...
 * bridges the node view's content into the page's light DOM. Here the content
 * is a plain `HTMLElement` (a self-styled `<cds-tile>`), so the chat appends
 * it directly — no React portal needed.
 *
 * APIs exercised:
...
const tileChipNodeImpl = Node.create({
  ...
  // (see `main.ts` `handleTileClick`) and travel with the node in the sent
  ...
  addNodeView() {
    return ({ node, editor }) => {
      ...
      // our content into the page's light DOM and hands back a `container` to
      // mount in the editor in its place. The content is a self-styled
      // `<cds-tile>` element, so it carries its own look across that boundary.
      const { container } = renderInLightDom({
        content: buildTileCard(
